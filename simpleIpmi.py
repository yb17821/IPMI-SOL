...
class SimpleIPMITOOL():
    ...
    def rawmsg_cut_pad(self, rawmsg):
        if self.debug:
            print('>>> rawmsg_cut_pad')
        padlength = rawmsg[-1]
        if padlength == 0:
            result = rawmsg[:-1]
        else:
    ...
    def analysis_rq(self, data):
        ...
        raw_msg_without_pad = self.rawmsg_cut_pad(raw_msg)
        ...
        if payload_type == 0xc0:
            ...
            if rsdata['pkt_seq'] != 0:
                self.send_solcmd(
                    pkt_seq=0x00,
                    ack_seq=rsdata['pkt_seq'],
                    recv_char_count=len(rsdata['data']),
                    status=0x00,
                    data=None
                )
        if self.debug:
    ...
    def close_session(self):
        ...
        completion_code, rqdata = si.sendipmicmd(
            netfn=0x06,
            cmd=0x3c,
            datalist=[
                self.SIDC[0],
...
if __name__ == '__main__':
    ...
    si = SimpleIPMITOOL('username', 'password', 'ip', debug=False)
    # create session
    si.setupsession()
    ...
    # close_session
    completion_code, rqdata = si.close_session()
    if completion_code != '00':
